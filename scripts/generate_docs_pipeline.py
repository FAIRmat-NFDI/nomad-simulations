...
def clean_old_docs(repo_root: Path) -> bool:
    ...
    try:
        ...
        if docs_schema_dir.exists():
            ...
            # Remove orphaned generated schema pages from old/renamed verticals.
            # These can remain after vertical refactors and cause broken-link warnings.
            known_verticals = set(VERTICALS.keys())
            for md_file in docs_schema_dir.glob('*.md'):
                if md_file.name == 'index.md':
                    continue

                if md_file.name.endswith('.diagram.md'):
                    vert_key = md_file.name[: -len('.diagram.md')]
                    is_diagram = True
                else:
                    vert_key = md_file.stem
                    is_diagram = False

                if vert_key in known_verticals:
                    continue
                ...
                if looks_generated:
                    md_file.unlink()
                    removed_count += 1
                    print(f'  Removed orphaned generated file: {md_file.name}')
        else:
            print('  docs/schema/ does not exist, skipping markdown cleanup')

        # Always clean autogenerated diagram assets (png/svg/temp mmd files)
        if assets_diagrams.exists():
            shutil.rmtree(assets_diagrams)
            print('  Removed: docs/assets/diagrams/')
        else:
            print('  No autogenerated diagram assets to remove')
...
def configure_diagram_zoom(repo_root: Path, method: str) -> bool:
    ...
    try:
        ...
        if method == 'panzoom':
            # Ensure panzoom plugin is present with explicit UX configuration.
            # NOTE: macOS "Option" key is the same as browser "Alt" modifier.
            panzoom_block = (
                '  - panzoom:\n'
                '      key: alt\n'
            ...
            had_panzoom = 'panzoom' in content

            if re.search(r'^\s*-\s*panzoom:\s*$', content, flags=re.MULTILINE):
                content = re.sub(
                    r'(?ms)^\s*-\s*panzoom:\s*\n(?:\s{4,}[^\n]*\n)*',
                    panzoom_block + '\n',
                    content,
            ...
            elif re.search(r'^\s*-\s*panzoom\s*$', content, flags=re.MULTILINE):
                content = re.sub(
                    r'^\s*-\s*panzoom\s*$',
                    panzoom_block,
                    content,
            ...
            content = re.sub(
                r'^(\s*(?:key|always_show_hint|hint_location|initial_zoom_level):.*?)$',
                r'  # \1',
                content,
                flags=re.MULTILINE,
            )
