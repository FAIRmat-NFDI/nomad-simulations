...
| `high_symmetry_points` | JSON | <details><summary>Dictionary containing the high-symmetry point labels and their values in units of `reciprocal_lattice_vectors`.</summary>Dictionary containing the high-symmetry point labels and their values in units of `reciprocal_lattice_vectors`.<br>E.g., in a cubic lattice:<br>high_symmetry_points = {<br>'Gamma': [0, 0, 0],<br>'X': [0.5, 0, 0],<br>'Y': [0, 0.5, 0],<br>...<br>}<br>**Symmetry source preference**: NOMAD uses the Bravais lattice classification from spglib (via MatID) as the<br>authoritative source for k-point generation, rather than ASE's geometric detection. This choice reflects the fact<br>that spglib analyzes both the lattice parameters and atomic positions during symmetry determination, while ASE's<br>`cell.get_bravais_lattice()` examines only the lattice geometry. In practice, the two analyses usually agree, but<br>near-symmetric structures can trigger disagreements—for example, a slightly distorted orthorhombic cell with<br>parameters within 0.3% of each other might be classified as cubic by ASE's tolerance-based detection, while spglib<br>identifies it as orthorhombic based on the full crystal structure. When such mismatches occur, NOMAD forces ASE to<br>generate k-points using spglib's classification, ensuring consistency between the stored `bravais_lattice` metadata<br>and the k-point mesh used in calculations.</details> |
