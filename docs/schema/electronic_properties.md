...
| `homo_lumo_gap_parsed` | m_float64(float64) | <details><summary>HOMO-LUMO gap as directly reported by the code.</summary>HOMO-LUMO gap as directly reported by the code. Strictly a parsed value: it is<br>not derived from `homo_parsed`/`lumo_parsed` (that computation lives on the<br>derived `homo_lumo_gap_normalized`). Leave unset if the code reports no gap<br>directly.</details> |
