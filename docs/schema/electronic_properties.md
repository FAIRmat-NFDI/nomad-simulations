...
| `homo_lumo_gap_parsed` | m_float64(float64) | HOMO-LUMO gap as directly reported by the code. Strictly a parsed value: it is not derived from `homo_parsed`/`lumo_parsed` (that computation lives on the resolved `homo_lumo_gap`). Leave unset if the code reports no gap directly. |
