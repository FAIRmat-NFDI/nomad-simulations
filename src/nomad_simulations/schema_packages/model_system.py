...
class ModelSystem(System):
    ...
    wyckoff_sites = Quantity(
        ...
        **International Tables for Crystallography** and accessible through resources like the **Bilbao 
        ...
        Each position is characterized by its site symmetry and multiplicity within the conventional unit cell.
        
        This annotation maps Wyckoff position assignments from the crystallographic conventional unit cell
        back to the atoms in the current structure, preserving the correspondence between atomic positions
        and their crystallographic classifications. Each element corresponds to the position at the same
        index in the `positions` array.

        The Wyckoff letter (a, b, c, ...) identifies positions in order of **decreasing site symmetry**, with 
        `a` typically representing the **highest symmetry** (most special) position. The **multiplicity** indicates 
        how many symmetrically equivalent positions are generated by applying all space group operations 
        to this Wyckoff site within the conventional unit cell.

        **Important**: 
        Wyckoff sites are determined using **geometric space group analysis** (via spglib/MatID),
        ...
        For complete crystallographic uniqueness, combine `wyckoff_sites` with chemical information.

        Examples:
        - In a conventional unit cell: ['a1', 'b2', 'b2', 'c4', 'c4', 'c4', 'c4'] indicates:
        ...
        - In a primitive unit cell of the same structure: ['a1', 'b2', 'c4'] might indicate:
          • The same 'a' position (special positions are typically preserved in primitive cells)
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        ...
        if (
            self.type == 'bulk' and self.symmetry is not None
        ):  # TODO: expand to lower dimensions
    ...
    def _compute_wyckoff_sites(self) -> list[str]:
        """
        Compute the Wyckoff sites from the positions and cell.

        Returns:
            list[str]: List of Wyckoff site annotations.
        """

        logger = self._compute_wyckoff_sites.__annotations__['logger']
        ase_atoms = self.to_ase_atoms(logger=logger)
        ...
        # Calculate multiplicities from conventional unit cell (inline)
        conventional_letter_counts = {
            letter: conventional_wyckoff.count(letter)
            for letter in set(conventional_wyckoff)
        }

        # Create wyckoff_sites annotations in format '<letter><multiplicity>' (inline)
        return [
            f'{letter}{conventional_letter_counts.get(letter, 1)}'
