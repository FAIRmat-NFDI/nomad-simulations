    ...
    def _populate_wyckoff_positions(self, symmetry_analyzer: 'SymmetryAnalyzer', logger: 'BoundLogger') -> None:
        ...
        try:
            ...
            for wyckoff_letter in sorted(letter_counts.keys()):
                ...
                # Get representative coordinates from the first occurrence (only if we have a proper cell)
                ...
                try:
                    ...
                    if ase_atoms and first_occurrence < len(ase_atoms):
                        ...
                        if not np.allclose(cell, 0):
                            # Convert to fractional coordinates (only meaningful for crystallographic systems)
                            frac_coords = np.linalg.solve(cell.T, pos)
                            wyckoff_pos.representative_coordinates = frac_coords
                        # Note: We don't set coordinates for non-crystallographic systems as 
                        # Wyckoff positions are only meaningful in the context of space groups
                except Exception as e:
                    logger.debug(f'Could not compute fractional coordinates for Wyckoff {wyckoff_letter}: {e}')
