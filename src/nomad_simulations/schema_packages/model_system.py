...
class WyckoffPosition(ArchiveSection):
    """
    Describes a unique Wyckoff position within a space group, providing comprehensive
    ...
    letter = Quantity(
        type=str,
        description="""
        Wyckoff letter (e.g., 'a', 'b', 'c', etc.) identifying this symmetry position
        within the space group. Each letter corresponds to a unique set of symmetry operations.
        """,
    )
    ...
    multiplicity = Quantity(
        type=np.int32,
        description="""
        Number of symmetry-equivalent positions generated by applying all space group
        operations to this Wyckoff position within the crystallographic conventional unit cell.
        """,
    )

    site_symmetry = Quantity(
        type=str,
        description="""
        Point group symmetry of this atomic site (site symmetry) in Hermann-Mauguin notation.
        Describes the local symmetry environment at this Wyckoff position.
        
        Examples: '1' (general position), 'mm2', '4mm', '-3m', 'm-3m'.
        """,
    )

    representative_coordinates = Quantity(
        type=np.float64,
        shape=[3],
        description="""
        Representative fractional coordinates for this Wyckoff position as defined
        in the International Tables for Crystallography. These are the standardized
        coordinates before applying symmetry operations.
        
        Examples: [0, 0, 0], [0.5, 0.5, 0.5], [0, 0.5, 0.25].
        """,
    )
    ...
    degrees_of_freedom = Quantity(
        type=np.int32,
        description="""
        Number of independent coordinate parameters that can vary for this Wyckoff position:
        - 0: fixed position (special position with fully constrained coordinates)
        - 1: one free parameter (e.g., position along a line: (x, 0, 0))
        - 2: two free parameters (e.g., position in a plane: (x, y, 0))
        - 3: three free parameters (general position: (x, y, z))
        """,
    )

    equivalent_coordinates = Quantity(
        type=np.float64,
        shape=['*', 3],
        description="""
        All symmetry-equivalent fractional coordinates generated by applying
        the space group operations to the representative coordinates. The number
        of coordinates equals the multiplicity.
        """,
    )

    coordinate_expressions = Quantity(
        type=str,
        shape=['*'],
        description="""
        Symbolic expressions for the equivalent coordinates showing free parameters.
        
        Examples: ['x,y,z', '-x,-y,z', 'y,-x,-z'] for a general position,
...
class Symmetry(ArchiveSection):
    ...
    wyckoff_positions = SubSection(
        sub_section=WyckoffPosition.m_def,
        repeats=True,
        description="""
        Detailed descriptions of all unique Wyckoff positions present in this structure.
        Each item describes one symmetry-distinct site type. The actual atomic
        positions reference these via the `ModelSystem.wyckoff_sites` annotations.
        """,
    )
